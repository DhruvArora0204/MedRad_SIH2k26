...
def test_full_stack_end_to_end_flow():
    ...
    # 5. Verify Frontend Static File Serving (if dist directory exists)
    frontend_res = client.get("/")
    assert frontend_res.status_code in [200, 404]
...
def test_png_image_upload_and_analyze():
    import cv2
    img = np.zeros((256, 256, 3), dtype=np.uint8)
    # Add a simulated bright lesion
    cv2.circle(img, (128, 128), 30, (200, 200, 200), -1)
    _, buf = cv2.imencode('.png', img)
    
    upload_res = client.post(
        "/api/v1/scans/upload",
        files={"file": ("brain_scan.png", buf.tobytes(), "image/png")}
    )
    assert upload_res.status_code == 201
    scan_id = upload_res.json()["scan_id"]
    assert upload_res.json()["metadata"]["Format"] == "PNG"
