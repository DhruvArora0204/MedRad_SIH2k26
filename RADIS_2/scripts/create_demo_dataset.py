...
def create_image_files(filename_base: str, pathology: str = "normal"):
    os.makedirs(DEMO_DIR, exist_ok=True)
    
    # Base Brain CT Pixel Array (HU 40 brain tissue = 1064 with intercept -1024)
    arr = np.full((256, 256), 40.0, dtype=np.float32)
    ...
    # Skull bone (1000 HU)
    mask_skull = (((x - 128)**2 + (y - 128)**2) <= 110**2) & (((x - 128)**2 + (y - 128)**2) >= 98**2)
    arr[mask_skull] = 1000.0

    # Add simulated pathology hyperdensities (80 HU bleed)
    if pathology == "epidural":
        mask_hem = (((x - 180)**2 + (y - 128)**2) <= 25**2) & (((x - 128)**2 + (y - 128)**2) < 98**2)
        arr[mask_hem] = 85.0
    elif pathology == "subdural":
    ...
    windowed = np.clip(arr, 0.0, 80.0) / 80.0
    img_uint8 = (windowed * 255.0).astype(np.uint8)
    img_uint8[mask_skull] = 255
    img_bgr = cv2.cvtColor(img_uint8, cv2.COLOR_GRAY2BGR)

    png_path = os.path.join(DEMO_DIR, f"{filename_base}.png")
    jpg_path = os.path.join(DEMO_DIR, f"{filename_base}.jpg")
    cv2.imwrite(png_path, img_bgr)
    cv2.imwrite(jpg_path, img_bgr)
    print(f"Created demo image: {png_path} and {jpg_path}")

def main():
    print("Generating demo CT scans (DICOM, PNG, JPG) in data/demo_scans/...")
    ...
    create_image_files("normal_brain_ct", "normal")
    create_image_files("epidural_hematoma_ct", "epidural")
    create_image_files("subdural_hematoma_ct", "subdural")
    create_image_files("subarachnoid_hemorrhage_ct", "subarachnoid")
