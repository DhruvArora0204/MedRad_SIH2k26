...
def test_upload_invalid_file():
    ...
    assert "Only image files (.png, .jpg, .jpeg) and DICOM" in response.json()["detail"]

def test_upload_valid_png_filename():
    import cv2
    import numpy as np
    ...
    response = client.post(
        "/api/v1/scans/upload",
        files={"file": ("brain_sample.png", buf.tobytes(), "image/png")}
    )
    assert response.status_code == 201
    data = response.json()
    assert "scan_id" in data
    assert data["filename"] == "brain_sample.png"
